...
@op_register("LinearQuant")
class LinearQuant(OpVectorBase):
    def __init__(self, chip_spec: ChipSpec):
        super().__init__("LinearQuant", chip_spec)
        self.inputs = []
        self.outputs = []
    ...
    def get_compute_cost(self) -> OpResult:
        # 对于LinearQuant，计算成本和内存成本可以使用相同的方法
...
@op_register("SwiGluQuant")
class SwiGluQuant(OpVectorBase):
    def __init__(self, chip_spec: ChipSpec):
        super().__init__("SwiGluQuant", chip_spec)
        self.inputs = []
        self.outputs = []
        self.op_dtype = "float16"  # 默认数据类型
    ...
    def get_compute_cost(self) -> OpResult:
        # 对于SwiGluQuant，计算成本和内存成本可以使用相同的方法
        return self.get_memory_cost()
...
@op_register("RMSNormQuant")
class RMSNormQuant(OpVectorBase):
    def __init__(self, chip_spec: ChipSpec):
        super().__init__("RMSNormQuant", chip_spec)
        self.inputs = []
        self.outputs = []
        self.op_dtype = "float16"  # 默认数据类型
    ...
    def get_compute_cost(self) -> OpResult:
        # 对于RMSNormQuant，计算成本和内存成本可以使用相同的方法
        return self.get_memory_cost()
...
@op_register("AddRMSNormQuant")
class AddRMSNormQuant(OpVectorBase):
    def __init__(self, chip_spec: ChipSpec):
        super().__init__("AddRMSNormQuant", chip_spec)
        self.inputs = []
        self.outputs = []
        self.op_dtype = "float16"  # 默认数据类型
    ...
    def get_compute_cost(self) -> OpResult:
        # 对于AddRMSNormQuant，计算成本和内存成本可以使用相同的方法
        return self.get_memory_cost()
