...
class CausalConvldUpdate(OpVectorBase):
    def __init__(self, chip_spec: ChipSpec):
        super().__init__("CausalConvldUpdate", chip_spec)
        self.inputs = []
        self.outputs = []
    ...
    def get_compute_cost(self) -> OpResult:
        # 对于CausalConvldUpdate，计算成本和内存成本可以使用相同的方法
...
class FusedSigmoidGatingDeltaRuleUpdate(OpVectorBase):
    def __init__(self, chip_spec: ChipSpec):
        super().__init__("FusedSigmoidGatingDeltaRuleUpdate", chip_spec)
        self.inputs = []
        self.outputs = []
        self.op_dtype = "float16"  # 默认数据类型
    ...
    def get_memory_cost(self) -> OpResult:
        ...
        # 计算计算成本（FusedSigmoidGatingDeltaRuleUpdate的计算量）
        total_compute_flops = 0
        if self.inputs:
            elements = self.inputs[0].numel
    ...
    def get_compute_cost(self) -> OpResult:
        # 对于FusedSigmoidGatingDeltaRuleUpdate，计算成本和内存成本可以使用相同的方法
        return self.get_memory_cost()
