...
- **流水线指标**：warmup / steady / cooldown 步数、bubble 占比 **与 bubble 绝对时间（ms）**
- **重计算时间**：激活重计算 fwd 重跑耗时单独成项（不计入 bwd compute）；未开重计算时为 0
- **时延分解**：见下表
...
| `compute_time_ms` | 关键路径纯计算 | `= fwd_compute + bwd_compute + recompute_time` |
| `fwd_compute_ms` / `bwd_compute_ms` | 前向 / 反向纯计算（`bwd` 已剔除重计算）| — |
| `recompute_time_ms` | 激活重计算 fwd 重跑（关键路径，从 bwd 拆出）| 未开重计算时 `= 0` |
| `bubble_ms` | 流水线空泡绝对时间 | `= warmup_ms + cooldown_ms` |
| `bubble_fraction` | 流水线空泡占比 | `= bubble_ms / step_time_ms` |
